...
import routerP from "../src/routers/products.routes.js";
...
describe("Test CRUD de productos en la ruta api/products", function () {
  it("iniciar sesion con post a traves de /sessions/login", async function () {
    ...
    const { _body, ok } = await requester.post("/sessions/login").send(admin);
    token = _body.token;
    expect(ok).to.be.ok;
  });
  ...
  it("Ruta: api/products/id metodo Get", async () => {
    const id = "653b161d5b041c5c74f2a9da";
  ...
  it("Ruta: api/products metodo Put", async () => {
    const id = "653b166f5b041c5c74f2a9de";
    const updateProduct = {
      title: "Nuevo Producto",
      description: "Descripción del nuevo producto",
      price: 25.99,
  ...
  it("Ruta: api/products/id metodo Delete", async () => {
    const id = "6584db007acdbbde32ac609b";
